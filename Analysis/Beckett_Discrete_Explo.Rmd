...
Right now I'm doing the animal pollinator benefits in a hacky forloop. There's a smart, fast way to do this, but I'm dumb atm and can't find it. 
```{r}
lambda <- 0.8
mua <- .1
...
```{r}
output <- data.frame(time=1, a_1=a_pops[1], a_2=a_pops[2], a_3=a_pops[3], p_1=p_pops[1], p_2=p_pops[2])
...
for(t in 1:100){
  ...
  output <- rbind(output, c(time=t+1, a_1=a_pops[1], a_2=a_pops[2], a_3=a_pops[3], p_1=p_pops[1], p_2=p_pops[2]))
}
```


```{r}
plot(x=output$time, y=output$a_1, col="pink")
points(x=output$time, y=output$a_2, col="red")
points(x=output$time, y=output$a_3, col="dark red")
points(x=output$time, y=output$p_1, col="green")
points(x=output$time, y=output$p_1, col="dark green")
```
