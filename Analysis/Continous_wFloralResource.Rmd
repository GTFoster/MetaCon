...
aP <- matrix(data=runif(length(plantState)^2, 0, 1/K), nrow=length(plantState)) #randomly draw interaction values, with max being equal to K
diag(aP) <- 1/K #set diags as max carrying capacity (20)

animalState <- c(5,1,5) # species community
aA <- matrix(data=runif(length(animalState)^2, 0, 1/K), nrow=length(animalState)) #randomly draw interaction values
