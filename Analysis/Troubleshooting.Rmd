...
```{r}
set.seed(24601)
disptype = "negativeComp"; n_plants = 5; n_animals = 5; dexpsim = dexpsim; r = 0.5; mup = 0.1; mua = 0.1; o = 0.1; lambda = 0.9; K = 100; e_thresh = 2; invade_size = 5; disprobmax = 0.2; num_timeSteps = 10000; invProb = 0.1
  ...
  for(t in 1:num_timeSteps){
    ...
    if(int_fail==TRUE){ #Same check for int_fail as above, but this time we'd be breaking our overall loop
      ...
      for(n in 1:n_animals){
        propOfplants <- 0
        nums <- p_pops %*%diag(alpha[n,])#Diag is important here; this essentially does column-wise multiplication
        for(i in 1:n_plants){
          #Numerator-plant reward given by each focal pollinator
          propOfplants <- propOfplants+nums[,i]/(r+colSums(a_pops %*% diag(alpha[,i]))) #Denominator-scaled by total available pollen
        }
        pollDeath <- mua
        #print(propOfplants)
