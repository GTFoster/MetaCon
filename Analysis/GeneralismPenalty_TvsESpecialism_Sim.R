...
#' @param genPen number; generalist species are penalized dependending on their interaction width. Their maximum alpha value with any plant is multiplied by their Variance
#' 
#' 
...
runDispersalSim <- function(X, disptype, nsites, dexpsim, disprobmax, n_plants, n_animals, r, mup, mua, o, lambda, K, e_thresh, invProb, invade_size, num_timeSteps=500, seedstart=0, genPen=0.5){
  ...
  p_traitV <- runif(n_plants, 0.05, .25)
  ...
  for(t in 1:num_timeSteps){
    ...
    if(richness > prior_richness){ #Check if we've updated the number of spp in the community
      ...
      #print("recalculating")
        ...
        alpha <- t(t(alpha) * scale_alpha(x=genPen, input=a_traitV)) # Apply our generalism penalty row-wise for pollinators
